...
/**
 * claude plugin marketplace add + claude plugin install 로 플러그인을 설치합니다.
...
export async function runInstaller(options = {}) {
  ...
  if (commandExists('sg')) {
    ...
    if (choice === 'brew') {
      ...
      console.log(colors.dim('  건너뜀. /claude-sdd:lint의 ast-grep 기능은 사용할 수 없습니다.'));
    }
  }

  // -- LSP Servers (optional) --
  console.log();
  console.log(colors.dim('  Language Server (LSP) — /claude-sdd:lsp 의미 분석에 사용'));
  console.log();
  ...
  if (isPluginInstalled()) {
    status('플러그인 등록', true, `${PLUGIN_NAME}@${PLUGIN_NAME} (이미 등록됨)`);
  ...
  console.log(`  2. ${colors.bold('/claude-sdd:init new')} 을 입력하여 SDD 프로젝트를 초기화하세요`);
  console.log(`  3. ${colors.bold('/claude-sdd:auto')} 를 사용하여 개발 라이프사이클을 시작하세요`);
  console.log();
  const cli = process.env.SDD_CLI_NAME || 'claude-sdd';
