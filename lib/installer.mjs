 ...
 * gopls가 이미 PATH에 있으면 패치하지 않습니다.
 */
async function configureGoplsPath() {
  // gopls가 이미 PATH에 있으면 패치 불필요
  if (commandExists('gopls')) return;
...
export async function runUninstaller() {
  ...
  try {
    const installed = readFileSync(join(home, '.claude', 'plugins', 'installed_plugins.json'), 'utf8');
    installedLsps = lspPlugins.filter(lsp => installed.includes(`"${lsp.plugin}@claude-code-lsps"`));
  } catch { /* ignore */ }

  if (installedLsps.length > 0) {
    section('LSP 플러그인 제거');
    const lspNames = installedLsps.map(l => l.label).join(', ');
    if (await confirm(`  ${sym.arr} LSP 플러그인을 제거하시겠습니까? (${lspNames})`)) {
      for (const lsp of installedLsps) {
