...
import {
  commandExists, run, prompt,
  colors, sym, status, section,
  isMac, refreshPath, ensureBrewNodeLinked,
} from './utils.mjs';
...
/**
 * Atlassian MCP 서버를 설정합니다.
 * 고정 2사이트 (lgdigitalcommerce, lgucorp), 이메일과 토큰만 입력받습니다.
 */
async function configureAtlassianMcp() {
  ...
  // 고정 2사이트
  const sites = [
    { name: 'mcp-atlassian-lgdigitalcommerce', url: 'https://lgdigitalcommerce.atlassian.net', label: 'LG Digital Commerce' },
    { name: 'mcp-atlassian-lgucorp', url: 'https://lgucorp.atlassian.net', label: 'LG U+ Corp' },
  ];

  console.log(colors.dim('  2개 사이트를 설정합니다:'));
  for (const site of sites) {
    console.log(colors.dim(`    - ${site.label} (${site.url})`));
  }
  console.log();

  const email = await prompt(`  사용자 이메일: `);
  if (!email) { console.log(colors.dim('  건너뜀.')); return; }
  const token = await prompt(`  API 토큰: `);
  if (!token) { console.log(colors.dim('  건너뜀.')); return; }
  ...
  for (const site of sites) {
    ...
    config.mcpServers[site.name] = {
      command: 'uvx',
      args: ['mcp-atlassian'],
      env,
    };

    status(`Atlassian MCP (${site.name})`, true, site.url);
  }
...
// ─────────────────────────────────────────────
// 다이어그램 도구 설정 (Mermaid 기반)
// ─────────────────────────────────────────────

/**
 * mmdc (Mermaid CLI) 설치 여부를 확인합니다.
 */
function hasMmdc() {
  try {
...
/**
 * 다이어그램 생성 도구를 설치합니다 (mmdc + claude-mermaid MCP + atlassian-python-api).
 */
async function configureDiagramTools() {
  // mmdc 확인
  const mmdc = hasMmdc();
  if (mmdc) {
    status('mmdc (Mermaid CLI)', true, '설치됨');
  }

  // atlassian-python-api 확인 (Confluence 첨부용)
  let hasAtlassianPy = false;
  try {
    const r = spawnSync('python3', ['-c', 'import atlassian'], { stdio: 'pipe', timeout: 10000 });
    hasAtlassianPy = r.status === 0;
  } catch { /* ignore */ }

  if (mmdc && hasAtlassianPy) {
    status('다이어그램 도구', true, 'mmdc + atlassian-python-api 설치됨');
    // claude-mermaid MCP 설정 확인 후 리턴
    await configureMermaidMcp();
    return;
  }
  ...
  if (!mmdc) {
    if (commandExists('npm')) {
      console.log(colors.dim('  npm i -g @mermaid-js/mermaid-cli 실행 중...'));
      const r = spawnSync('npm', ['i', '-g', '@mermaid-js/mermaid-cli'], { stdio: 'inherit', timeout: 300000 });
      if (r.status === 0) {
        status('mmdc (Mermaid CLI)', true, '설치 완료');
      } else {
        status('mmdc (Mermaid CLI)', false, '설치 실패');
      }
    } else {
      status('mmdc (Mermaid CLI)', false, 'npm 없음 — npm i -g @mermaid-js/mermaid-cli');
    }
  }

  // atlassian-python-api 설치 (Confluence PNG 첨부용)
  if (!hasAtlassianPy) {
    if (commandExists('pip3')) {
      console.log(colors.dim('  pip3 install atlassian-python-api 실행 중...'));
      ...
      if (r.status === 0) {
        status('atlassian-python-api', true, '설치 완료');
      } else {
        status('atlassian-python-api', false, '설치 실패');
      }
    } else {
      status('atlassian-python-api', false, 'pip3 없음 — 수동 설치 필요');
    }
  } else {
    status('atlassian-python-api', true, '이미 설치됨');
  }

  // claude-mermaid MCP 설정 (선택)
  await configureMermaidMcp();
}

/**
 * claude-mermaid MCP 서버를 설정합니다.
 */
async function configureMermaidMcp() {
  const config = readClaudeJson();
  const mcpServers = config.mcpServers || {};
  const hasMermaidMcp = Object.keys(mcpServers).some(k => k.includes('mermaid'));

  if (hasMermaidMcp) {
    status('claude-mermaid MCP', true, '이미 설정됨');
  } else {
    config.mcpServers = config.mcpServers || {};
    config.mcpServers['claude-mermaid'] = {
      command: 'npx',
      args: ['-y', 'claude-mermaid'],
    };
    writeClaudeJson(config);
    status('claude-mermaid MCP', true, '설정 완료');
  }
}
...
export async function runInstaller() {
  ...
  if (isAgentTeamsEnabled()) {
    ...
    const enableTeamsAnswer = await prompt('  Agent Teams를 활성화하시겠습니까? (Y/n): ');
    if (enableTeamsAnswer.trim().toLowerCase() === 'n') {
      status('Agent Teams', false, '비활성화 (사용자 선택)');
      console.log(colors.dim(`  ${sym.arr} 나중에 활성화: settings.json에 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 추가`));
    } else {
      try {
  ...
  if (ghInstalled) {
    ...
    if (ghAuth) {
      ...
      console.log(colors.dim('  gh auth login 실행 중...'));
  ...
  // brew로 gh 설치 후 node symlink 보호
  ensureBrewNodeLinked();

  // -- Step 4: Code Analysis Tools (optional) --
  section(colors.bold('[4/7] 코드 분석 도구'));
  ...
  if (commandExists('sg')) {
    ...
    if (isMac && commandExists('brew')) {
      ...
      status('ast-grep (sg)', false, '패키지 매니저 없음');
    }
  }
  ...
  if (!commandExists('claude')) {
    ...
    // brew로 LSP 서버 설치 후 node symlink 보호
    ensureBrewNodeLinked();
  ...
  section(colors.bold('[5/7] MCP 서버 설정'));

  await configureMcpServers();

  // -- Step 6: Diagram Tools (optional) --
  section(colors.bold('[6/7] 다이어그램 도구'));

  await configureDiagramTools();

  // brew로 다이어그램 도구 설치 후 node symlink 보호
  ensureBrewNodeLinked();

  // -- Step 7: Plugin Registration --
  section(colors.bold('[7/7] 플러그인 등록'));

  const regResult = registerPlugin();
  const pluginRegistered = regResult.marketplace && regResult.plugin;
