...
export async function runUninstaller() {
  ...
  const installed = {
    ...
    tools: {
      ...
      gh: commandExists('gh'),
      temurin21: isMac && existsSync('/Library/Java/JavaVirtualMachines/temurin-21.jdk'),
    },
  };

  // LSP 플러그인 확인
  const lspPlugins = [
    { label: 'TypeScript / JavaScript', plugin: 'vtsls', bin: 'vtsls',
      uninstall: { npm: '@vtsls/language-server typescript' } },
    { label: 'Python', plugin: 'pyright', bin: 'pyright',
      uninstall: { pip: 'pyright' } },
    { label: 'Go', plugin: 'gopls', bin: 'gopls',
      uninstall: { rm: join(home, 'go', 'bin', 'gopls') } },
    { label: 'Java', plugin: 'jdtls', bin: 'jdtls',
      uninstall: { brew: 'jdtls' } },
    { label: 'Kotlin', plugin: 'kotlin-lsp', bin: 'kotlin-lsp',
      uninstall: { brew: 'JetBrains/utils/kotlin-lsp' } },
    { label: 'Lua', plugin: 'lua-language-server', bin: 'lua-language-server',
      uninstall: { brew: 'lua-language-server' } },
    { label: 'Terraform', plugin: 'terraform-ls', bin: 'terraform-ls',
      uninstall: { brew: 'terraform-ls' } },
    { label: 'YAML', plugin: 'yaml-language-server', bin: 'yaml-language-server',
      uninstall: { npm: 'yaml-language-server' } },
  ];
  ...
  if (installed.lsps.length > 0) {
    targets.push(`LSP 플러그인 + 서버 바이너리 (${installed.lsps.length}개): ${installed.lsps.map(l => l.plugin).join(', ')}`);
  }
  ...
  const extraTools = [];
  if (installed.tools.uv) extraTools.push('uv');
  if (installed.tools.astGrep) extraTools.push('ast-grep');
  if (installed.tools.gh) extraTools.push('gh');
  if (installed.tools.temurin21) extraTools.push('temurin@21');
  if (extraTools.length > 0) targets.push(`추가 도구: ${extraTools.join(', ')}`);
  ...
  if (installed.lsps.length > 0) {
    ...
    for (const lsp of installed.lsps) {
      ...
      // 2-b. LSP 서버 바이너리 제거
      if (lsp.uninstall && commandExists(lsp.bin)) {
        const u = lsp.uninstall;
        let binRemoved = false;
        if (u.npm && commandExists('npm')) {
          const r = spawnSync('npm', ['uninstall', '-g', ...u.npm.split(' ')], { stdio: 'pipe', timeout: 60000 });
          binRemoved = r.status === 0;
        } else if (u.pip && commandExists('pip3')) {
          const r = spawnSync('pip3', ['uninstall', '-y', '--break-system-packages', u.pip], { stdio: 'pipe', timeout: 60000 });
          binRemoved = r.status === 0;
        } else if (u.brew && isMac && commandExists('brew')) {
          const r = spawnSync('brew', ['uninstall', u.brew], { stdio: 'pipe', timeout: 60000 });
          binRemoved = r.status === 0;
        } else if (u.rm && existsSync(u.rm)) {
          try { unlinkSync(u.rm); binRemoved = true; } catch { /* ignore */ }
        }
        if (binRemoved) {
          status(`${lsp.label} 서버`, true, '바이너리 제거 완료');
        } else {
          status(`${lsp.label} 서버`, false, '바이너리 제거 실패 — 수동 제거 필요');
        }
      }
    }
  ...
  // 5. 추가 도구 제거 (uv, ast-grep, gh, temurin@21)
  if (extraTools.length > 0) {
    section('추가 도구 제거');

    if (installed.tools.uv && isMac && commandExists('brew')) {
      const r = spawnSync('brew', ['uninstall', 'uv'], { stdio: 'pipe', timeout: 60000 });
      status('uv', r.status === 0, r.status === 0 ? '제거 완료' : '제거 실패');
      if (r.status === 0) removed = true;
    }

    if (installed.tools.astGrep) {
      let astRemoved = false;
      // npm으로 설치된 경우
      if (commandExists('npm')) {
        const r = spawnSync('npm', ['uninstall', '-g', '@ast-grep/cli'], { stdio: 'pipe', timeout: 60000 });
        if (r.status === 0) astRemoved = true;
      }
      // brew로 설치된 경우
      if (!astRemoved && isMac && commandExists('brew')) {
        const r = spawnSync('brew', ['uninstall', 'ast-grep'], { stdio: 'pipe', timeout: 60000 });
        if (r.status === 0) astRemoved = true;
      }
      status('ast-grep', astRemoved, astRemoved ? '제거 완료' : '제거 실패');
      if (astRemoved) removed = true;
    }

    if (installed.tools.gh && isMac && commandExists('brew')) {
      const r = spawnSync('brew', ['uninstall', 'gh'], { stdio: 'pipe', timeout: 60000 });
      status('gh', r.status === 0, r.status === 0 ? '제거 완료' : '제거 실패');
      if (r.status === 0) removed = true;
    }

    if (installed.tools.temurin21 && isMac && commandExists('brew')) {
      const r = spawnSync('brew', ['uninstall', '--cask', 'temurin@21'], { stdio: 'pipe', timeout: 60000 });
      status('temurin@21', r.status === 0, r.status === 0 ? '제거 완료' : '제거 실패');
      if (r.status === 0) removed = true;
    }
  }

  // 6. 마켓플레이스 제거 (renumbered)
