...
export function checkAll() {
  ...
  const hasGraphviz = commandExists('dot');
  results.push({
    name: 'Graphviz (dot)',
    ok: hasGraphviz,
    detail: hasGraphviz ? '설치됨' : '설치되지 않음 (선택 사항 — 다이어그램 생성)',
    category: 'tools',
  });

  let hasDiagramsPy = false;
  try {
    const r = spawnSync('python3', ['-c', 'import diagrams'], { stdio: 'pipe', timeout: 10000 });
    hasDiagramsPy = r.status === 0;
  } catch { /* ignore */ }
  results.push({
    name: 'Python diagrams',
    ok: hasDiagramsPy,
    detail: hasDiagramsPy ? '설치됨' : '설치되지 않음 (선택 사항 — 아키텍처 다이어그램)',
    category: 'tools',
  });
...
export function printResults(results) {
  const categories = [
    ...
    { key: 'tools',   title: '[4/7] 코드 분석 / 다이어그램 도구 (선택 사항)' },
    { key: 'mcp',     title: '[5/7] MCP 서버 (선택 사항)' },
  ];
