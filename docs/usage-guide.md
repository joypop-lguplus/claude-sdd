...
SDD의 핵심 단계입니다. Agent Teams 활성화 여부에 따라 자동으로 실행 모드가 결정됩니다:

- **팀 모드** (Agent Teams 활성화): 같은 stage의 워크 패키지를 병렬로 실행
- **솔로 모드** (Agent Teams 비활성화): 현재 세션이 워크 패키지를 순차적으로 직접 구현

```
[팀 모드]                          [솔로 모드]
리더가 워크 패키지 할당              에이전트 규칙 로드
  |                                  |
팀 멤버가 병렬로 구현               WP를 순차적으로 직접 구현
  |                                  |
체크리스트 항목 검증                 체크리스트 항목 검증
  |-- 미완료? --> 재작업               |-- 미완료? --> 직접 재구현
  |-- 완료? --> 다음 단계              |-- 완료? --> 다음 WP
  |-- 3회 실패? --> 에스컬레이션       |-- 3회 실패? --> 에스컬레이션
```
