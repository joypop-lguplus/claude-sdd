...
### Q: Agent Teams 없이도 사용할 수 있나요?

네. Agent Teams가 비활성화되면 **솔로 모드**로 자동 전환됩니다. 솔로 모드에서는 현재 세션이 워크 패키지를 순차적으로 직접 구현합니다. 결과물(코드, 테스트, 체크리스트)은 팀 모드와 동일하며, 실행 방식만 다릅니다(병렬 vs 순차). Agent Teams를 활성화하면 병렬 빌드로 더 빠르게 진행할 수 있습니다.

### Q: 빌드 중 에러가 나면?

최대 3회까지 재작업이 시도됩니다 (팀 모드: 리더가 멤버에게 지시, 솔로 모드: 직접 재구현). 3회 실패 시 사용자에게 에스컬레이션됩니다. 직접 개입하여 코드를 수정한 뒤 `/claude-sdd:sdd-build`를 다시 실행할 수 있습니다.
