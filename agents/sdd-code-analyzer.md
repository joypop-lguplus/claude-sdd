...
이 에이전트에는 `haiku`를 사용합니다. 외부 도구 실행 및 결과 수집이 주 역할이므로 경량 모델로 충분합니다.
...
분석은 다음 단계를 **순서대로** 실행합니다. 각 단계의 결과를 보고서에 포함합니다.
...
```
결과를 기반으로 이후 단계에서 사용할 도구를 결정합니다.
...
| 도구 | 상태 | 비고 |
|------|------|------|
| sdd-detect-tools.sh | ✓ 실행 | Java 25, Gradle 감지 |
| 네이티브 진단 (gradle) | ✓ 실행 | 컴파일 에러 0건 |
| ast-grep | ✗ 미설치 | grep 대체 사용 |
| 포매터 | ✓ 실행 | 2개 파일 미포맷 |
...
3. **네이티브 도구를 사용합니다.** 프로젝트의 네이티브 진단/린트 도구를 직접 실행합니다.
