...
```
/sdd-build                 # 빌드 단계 시작/재개
/sdd-build wp-1            # 특정 워크 패키지만 빌드
/sdd-build wp-1 rework     # 특정 패키지에 피드백 기반 재작업
```
...
1. **프로젝트 포매터 실행** (설정된 경우): 수정된 파일 자동 포맷
   - `/sdd-lint format --fix` 또는 프로젝트에 설정된 포매터 사용
2. **프로젝트 린터 실행** (설정된 경우): 린트 에러 확인
   - `/sdd-lint diagnostics` 또는 프로젝트에 설정된 진단 도구 사용
...
이 단계는 권장 사항이지만 필수는 아닙니다. `/sdd-review` 품질 게이트에서 나머지 문제를 잡아냅니다.
