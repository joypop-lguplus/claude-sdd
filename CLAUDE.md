...
```
/sdd           → 단계 자동 감지 후 계속 진행
/sdd-init      → 프로젝트 설정 + SDD 디렉토리 초기화
/sdd-intake    → 요구사항 수집 (Confluence/Jira/Figma/파일/인터뷰)
/sdd-spec      → 기술 스펙 + 스펙 준수 체크리스트 생성
/sdd-plan      → 태스크 분해 → 워크 패키지 + 팀 할당
/sdd-build     → Agent Teams로 구현 + 품질 루프 (최대 3회 재작업 사이클)
/sdd-review    → 품질 게이트 검증 + 자동 진단
...
### 템플릿 (`templates/`)
- `claude-md/` -- `/sdd-build` 시 대상 프로젝트에 주입되는 CLAUDE.md 템플릿 (리더 vs 멤버 규칙)
